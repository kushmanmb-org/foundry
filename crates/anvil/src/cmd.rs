...
mod tests {
    ...
    #[test]
    fn can_parse_max_transactions() {
        let args: NodeArgs = NodeArgs::parse_from(["anvil", "--max-transactions", "500"]);
        assert_eq!(args.max_transactions, Some(500));
        let config = args.into_node_config().unwrap();
        assert_eq!(config.max_transactions, 500);
    }
    ...
    fn can_parse_enable_tx_gas_limit() {
        ...
        let config = args.into_node_config().unwrap();
        assert!(config.enable_tx_gas_limit);
    }
}
